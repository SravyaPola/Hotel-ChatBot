...
public class SlotUtil {
	...
	public static void applySlots(ConversationState st, Map<String, String> slots, HotelService hotelService) {
		...
		slots.forEach((k, v) -> {
			...
			case "checkIn" -> {
				try {
					st.setCheckIn(LocalDate.parse(v));
				} catch (Exception ignored) {
				}
			}
			case "checkOut" -> {
				try {
					st.setCheckOut(LocalDate.parse(v));
				} catch (Exception ignored) {
				}
			}
			case "guests" -> {
				try {
					st.setGuests(Integer.valueOf(v.trim()));
				} catch (Exception ignored) {
				}
			}
			case "minStars" -> {
			...
			case "hotelId" -> {
				try {
					st.setHotelId(Integer.valueOf(v));
				} catch (Exception ignored) {
				}
			}
			...
			case "noRooms" -> {
				try {
					st.setNoRooms(Integer.valueOf(v));
				} catch (Exception ignored) {
				}
			}
			...
			case "confirmBooking" -> {
				try {
					st.setConfirmBook(Boolean.valueOf(v));
				} catch (Exception ignored) {
				}
			}
			case "confirmAnother" -> {
				try {
					st.setConfirmAnother(Boolean.valueOf(v));
				} catch (Exception ignored) {
				}
			}
			case "confirmAddAnother" -> {
				try {
					st.setConfirmAddAnother(Boolean.valueOf(v));
				} catch (Exception ignored) {
				}
			}
			case "confirmPayment" -> {
				try {
					st.setPaymentDone(Boolean.valueOf(v));
				} catch (Exception ignored) {
				}
			}
			case "feedbackRating" -> {
				try {
					st.setFeedbackRating(Integer.valueOf(v));
				} catch (Exception ignored) {
				}
			}
